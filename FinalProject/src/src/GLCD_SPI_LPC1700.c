...
void GLCD_Bitmap_Yfliped (unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned char *bitmap) {
  int i, j;
  unsigned short *bitmap_ptr = (unsigned short *)bitmap;
...
void GLCD_Bitmap_XYfliped (unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned char *bitmap) {
  int i, j;
  unsigned short *bitmap_ptr = (unsigned short *)bitmap;
...
void GLCD_Bitmap_ROT (unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned int rot, unsigned char *bitmap) {
  int i, j;
  unsigned short *bitmap_ptr = (unsigned short *)bitmap;

  GLCD_SetWindow (x, y, h, w);

  wr_cmd(0x22);
  wr_dat_start();
  for (i = 0; i<w; i++) {
    for (j = 0 ; j<h ; j++) {
      switch(rot&3)//|DIRECTION: ..00:x++ ..01:y++ ..10:x-- ..11:y-- 
      {
      	case 0: 
			wr_dat_only (bitmap_ptr[i*w+j]);
			break;
		case 1:
			wr_dat_only (bitmap_ptr[j*h+i]);
			break;
		case 2:
			wr_dat_only (bitmap_ptr[(w-i-1)*w+(h-j-1)]);			
			break;
		case 3:
		    wr_dat_only (bitmap_ptr[(h-j-1)*h+(w-i-1)]);
			break;
      }
    }
  }
